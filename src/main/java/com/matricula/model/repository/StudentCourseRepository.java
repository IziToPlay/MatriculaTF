...
public interface StudentCourseRepository extends JpaRepository<StudentCourse, Long> {
	...
	@Query("SELECT sc FROM StudentCourse sc WHERE sc.course.id=?1 AND sc.student.id=?2 AND sc.enrollment.semester=?3")
	List<StudentCourse> CourseStudentRegistered(Long idCourse, Long studentId,Integer semester);
